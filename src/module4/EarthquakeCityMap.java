...
public class EarthquakeCityMap extends PApplet {
	...
	private void addKey() {
		...
		final int H_MARGIN = 15;
		final int V_MARGIN = 20;

		fill(128, 9, 52);
		int x = 55;
		int y = 100;
		int height = 10;
		triangle(x - height / 2, y + height / 2, x, y - height / 2, x + height / 2, y + height / 2);
		fill(0, 0, 0);
		text("City Marker", x + H_MARGIN, y);

		fill(255, 255, 255);
		ellipse(x, y + V_MARGIN, height, height);
		fill(0, 0, 0);
		text("Land Quake", x + H_MARGIN, y + V_MARGIN);

		fill(255, 255, 255);
		rect(x - height / 2, y + V_MARGIN + V_MARGIN - height / 2, height, height);
		fill(0, 0, 0);
		text("Ocean Quake", x + H_MARGIN, y + V_MARGIN + V_MARGIN);

		text("Size ~ Magnitude", x - 8, y + V_MARGIN + V_MARGIN + V_MARGIN);

		fill(255, 255, 0);
		ellipse(x, y + 5 * V_MARGIN, height, height);
		fill(0, 0, 0);
		text("Shallow", x + H_MARGIN, y + 5 * V_MARGIN);

		fill(0, 0, 155);
		ellipse(x, y + 6 * V_MARGIN, height, height);
		fill(0, 0, 0);
		text("Intermediate", x + H_MARGIN, y + 6 * V_MARGIN);

		fill(255, 0, 0);
		ellipse(x, y + 7 * V_MARGIN, height, height);
		fill(0, 0, 0);
		text("Deep", x + H_MARGIN, y + 7 * V_MARGIN);
	...
	private void printQuakes() {
		...
		for (Marker cm : countryMarkers) {
			String name = (String) cm.getProperty("name");
			int landQuakesCounts = 0;
			for (Marker qm : quakeMarkers) {
				EarthquakeMarker em = (EarthquakeMarker) qm;
				if (em.isOnLand) {
					if (em.getProperty("country").equals(name)) {
						landQuakesCounts++;
					}
				}
			}
			if (landQuakesCounts > 0) {
				System.out.println(name + ": " + landQuakesCounts);
			}
		}
