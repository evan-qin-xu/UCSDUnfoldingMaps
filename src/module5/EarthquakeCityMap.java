...
public class EarthquakeCityMap extends PApplet {
	...
	private void selectMarkerIfHover(List<Marker> markers) {
		for (Marker marker : markers) {
			...
			if (commonMarker.isInside(map, mouseX, mouseY) && lastSelected == null) {
	...
	public void mouseClicked() {
		...
		if (lastClicked != null) {
			// de-selects whichever marker was clicked on last so that all
			// markers
			// should be displayed
			lastClicked = null;
			unhideMarkers();
			...
			// hide/display other markers outside of threat circles
			checkQuakeMarkersForClick();
			if (lastClicked == null) {
				checkCityMarkersForClick();
			}
		}
	}

	private void checkCityMarkersForClick() {
		for (Marker cm : cityMarkers) {
			if (cm.isInside(map, mouseX, mouseY) && !cm.isHidden()) {
				...
				for (Marker qm : quakeMarkers) {
					double threatCircle = ((EarthquakeMarker) qm).threatCircle();
					if (qm.getDistanceTo(cm.getLocation()) > threatCircle) {
						qm.setHidden(true);
					}
				}
				return;
	...
	private void checkQuakeMarkersForClick() {
		for (Marker qm : quakeMarkers) {
			if (qm.isInside(map, mouseX, mouseY) && !qm.isHidden()) {
				double threatCircle = ((EarthquakeMarker) qm).threatCircle();
				lastClicked = (CommonMarker) qm;
				for (Marker cm : cityMarkers) {
					if (cm.getDistanceTo(qm.getLocation()) > threatCircle) {
						System.err.println("threatCircle 2: " + threatCircle);
						cm.setHidden(true);
					}
				}
